...
// 모달 상세 내역
const IngredientModalComponent = ({ isOpen, onClose, ingredientId, onSubmit }) => {
    // 모달 창 내에서의 상태 관리
    ...
    useEffect(() => {
      axios.get(`/option/ingredients/${ingredientId}`)
        .then(response => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const response = await axios.put(`/option/ingredients/info/${ingredientId}`, {
          ingredientName,
          ingredientUnit
        });
    ...
    return (
      ...
      <form className='modal-form' onSubmit={handleSubmit}>
        ...
        <label>
          재료 단위 <br></br>
          <input type="text" value={ingredientUnit} onChange={(e) => setIngredientUnit(e.target.value)} />
        </label>
        <br/>
        </div>
